...
{\smallbody A public synthetic-fixture demonstrator in which a reviewer drags geometry parameters, sees the nearest stored wing and its $C_l/C_d$ update live, and releases to snap every control to that exact row. Public scientific outputs are limited to geometry/parameters and $C_l/C_d$; all SPOD/modal values are stripped and not rendered. The interface never predicts or interpolates aerodynamic values.}
\end{callout}
...
\begin{itemize}
  ...
  \item \textbf{Publication safety:} credentials, local paths, W\&B namespace, and every modal value must remain outside the public artifact.
...
{\smallbody
The selected delivery target is a \textbf{public GitHub Pages} project site, not an authentication or email-allowlist boundary. Main/manual deployment replays only the committed fixture with \texttt{--exclude-modal-data} and needs no secret. A separate hourly/manual W\&B workflow remains disabled unless \texttt{PUBLIC\_RESEARCH\_DATA\_APPROVED=true} and an API key is present; enabling it makes the same restricted profile public. Expected URL: \href{https://edengol10.github.io/AI-final-course/}{edengol10.github.io/AI-final-course/} (not yet verified). References: \href{https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages}{GitHub Pages workflows}, \href{https://docs.github.com/en/pages/getting-started-with-github-pages/changing-the-visibility-of-your-github-pages-site}{Pages visibility}, and \href{https://docs.wandb.ai/models/track/public-api-guide}{W\&B Public API}.
}
...
{\smallbody
Committed fixture $\rightarrow$ modal stripping $\rightarrow$ canonical JSON/SHA-256 $\rightarrow$ Vite subpath build and privacy scan $\rightarrow$ Pages artifact $\rightarrow$ public GitHub Pages $\rightarrow$ browser Zod/hash validation and Web Worker search. The optional approved W\&B path performs narrow export and admission before joining the same restricted publication path.
}
...
{\smallbody
The manifest's explicit \texttt{snapshotKind} is either \texttt{synthetic-fixture} or \texttt{reviewed-wandb}; the interface uses it to label fixture evidence visibly. Public workflows also require \texttt{modalDataIncluded=false}. The approved scientific outputs are BP3333 geometry/parameters and per-wing $C_l/C_d$; every SPOD/modal value is stripped and not rendered. Minimal run/step provenance, replicate count, curvature/admission metadata, compatibility descriptors, bounds, counts, and checksums remain for auditability; W\&B entity/project names and per-row coordinate arrays are absent.
}
...
\begin{itemize}
  ...
  \item Corrected private frequency semantics, then removed every SPOD/modal value from the approved public Pages profile.
...
\begin{callout}[colback=white]
{\smallbody\textbf{Observed value of the custom skill.} The major improvement was not writing speed; it was eliminating plausible scientific errors in field choice, normalization, private frequency interpretation, grouping, and replicate provenance before data crossed the publication boundary.}
\end{callout}
...
Exporter & 44 Python tests; deterministic fixture, grouping, rejection, and restricted profile & \statuspassed \\
...
GitHub Pages & Workflow prepared; Settings, deploy, URL, and browser check pending & \statusblocked \\
\end{tabularx}
}
...
\begin{itemize}[leftmargin=1.15em,itemsep=1pt,topsep=1pt]
  ...
  \item Confirm the artifact and UI contain no SPOD/modal values.
