...
2. **Fetch the narrow history.** Use the server-side W&B Public API and `scan_history` with only the approved per-wing keys. For the public Cl/Cd profile, do not request either SPOD key. Do not download artifacts, media, models, system metrics, run config dumps, or full metadata.
...
8. **Publish compactly.** Validate models with Pydantic, serialize columnar data, split hashed files below 10 MiB, and calculate SHA-256 over canonical bytes. Mark the manifest `snapshotKind` as either `synthetic-fixture` or `reviewed-wandb`; set `modalDataIncluded=false` for the public Cl/Cd profile and reject every non-null modal value. Keep the W&B entity/project in the private registry rather than the public manifest. Validate a complete staging tree, then replace the output directory as one clean swap so stale shards cannot survive. Restore the previous verified tree if the swap fails.
...
- A modal-enabled private snapshot interprets its two frequencies as the first and second ranked peaks of SPOD mode 1, never “mode 1” and “mode 2.” The public GitHub Pages profile requests, exports, and renders no modal values.
- A UI metric tuple must always point to one admitted provenance record. Never interpolate or predict `Cl`, `Cd`, or any private frequency value.
