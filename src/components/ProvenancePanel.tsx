...
import { BASELINE_PARAMETERS, PARAMETER_BY_NAME, PARAMETER_ORDER, formatParameter } from "../domain/parameters";
...
export function ProvenancePanel({ record, group, manifest }: { record: WingRecord | null; group: DatasetGroup; manifest: SnapshotManifest }) {
  ...
  return (
    <section className="card provenance-card" aria-labelledby="provenance-title">
      ...
      {record ? (
        ...
        <div className="reference-provenance" role="note">
          <strong>Reference definition · no measured run/step.</strong>
          <span>Measured provenance and aerodynamic response are intentionally unavailable.</span>
        </div>
      )}
      ...
      <details>
        ...
        <dl className="fixed-grid">
          {fixed.map((parameter) => (
            <div key={parameter}><dt>{PARAMETER_BY_NAME[parameter].symbol}</dt><dd>{formatParameter(record?.parameters[parameter] ?? BASELINE_PARAMETERS[parameter])}</dd></div>
          ))}
        </dl>
      </details>
    </section>
